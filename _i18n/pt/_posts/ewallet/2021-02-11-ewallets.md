---
layout: redirect
redirect: https://docs.cielo.com.br/gateway/docs/wallets
title: E-Wallets
...
[![Novo portal de desenvolvedores e-commerce Braspag e Cielo]({{ site.baseurl_root }}/images/novo-docs.cielo.com.br.png)](https://docs.cielo.com.br/gateway/docs/wallets)

Acesse o novo portal de desenvolvedores E-commerce **[docs.cielo.com.br](https://docs.cielo.com.br/gateway/docs/wallets)**.

> **Atenção**: O conteúdo desta página está sendo descontinuado e não receberá atualizações a partir de 14/08/2024. Visite a nova documentação em [docs.cielo.br](https://docs.cielo.com.br/gateway/docs/wallets).
...
> **Bandeiras aceitas pelas wallets**<br>
> Para saber as bandeiras de cartão aceitas, visite o site de cada carteira:<br>
...
> [SamsungPay](https://www.samsung.com.br/services/pay/){:target="\_blank"}<br>
> [PayPal](PayPal: https://www.paypal.com/br/cshelp/article/help468){:target="\_blank"}
...
| `Wallet.Type`|Tipo de carteira: "ApplePay" / "SamsungPay" / "GooglePay".|Texto|--|Sim|
...
| `Type`              | Tipo de carteira: "ApplePay" / "SamsungPay" / "GooglePay".                      | Texto | --     | Texto alfanumérico                   |
| `WalletKey`         | Chave criptográfica que identifica lojas nas wallets. Consulte a tabela [WalletKey](https://braspag.github.io//manual/ewallets#walletkey) para mais informações.                              | Texto | --     | Ver tabela [WalletKey](https://braspag.github.io//manual/braspag-pagador#walletkey){:target="_blank"}               |       
| `AdditionalData.EphemeralPublicKey` | Token retornado pela wallet. Deve ser enviado em Integrações: "ApplePay".                                       | Texto | --     | Ver tabela [EphemeralPublicKey](https://braspag.github.io//manual/braspag-pagador#ephemeralpublickey){:target="_blank"}      |  
| `AdditionalData.Signature` | Token retornado pela wallet. Deve ser enviado em Integrações: "GooglePay".                                               | Texto | --     | Ver tabela [Signature](https://braspag.github.io//manual/braspag-pagador#signature){:target="_blank"}      |  
{:target="_blank"}
...
> Se a sua loja envia a requisição de autorização de e-wallet com dados descriptografados, vá para [Cartão descriptografado](https://braspag.github.io//manual/ewallets?json#cart%C3%A3o-descriptografado)
...
1. Realizar o cadastro, acessando [esta URL](https://developer.apple.com/programs/enroll/){:target="_blank"} e seguindo todos os passos requeridos pela Apple.
...
5. Integrar com o [Pagador API REST](https://braspag.github.io/manual/braspag-pagador){:target="_blank"}.
...
A criação do merchant identifier deve ser feita através do [Portal de Desenvolvedores da Apple](https://developer.apple.com/account/resources){:target="_blank"}, da seguinte maneira:
...
Para solicitar à Braspag a geração do certificado no formato ".CSR", entre em contato com o nosso [Time de Atendimento](https://suporte.braspag.com.br/hc/pt-br/requests/new){:target="_blank"} e informe:
...
> O campo `Payment.Wallet.WalletKey` na requisição de autorização do Pagador deverá ser preenchido com o valor enviado pela Apple em `paymentData.data`.
...
| `Customer` |  Nó com dados do comprador. |Consulte o [Manual do Pagador](https://braspag.github.io/manual/braspag-pagador?json#requisi%C3%A7%C3%A3o){:target="_blank"}. |
...
A resposta de autorização da wallet terá os mesmos campos apresentados em nossa documentação do [Pagador](https://braspag.github.io//manual/braspag-pagador#resposta){:target="_blank"}, porém com a adição do nó `Payment.Wallet` repetindo os mesmos campos utilizados na autorização, como descrito acima.
...
Para configurar seu projeto, siga as instruções descritas no [Guia de Configuração](https://developers.google.com/pay/api/android/guides/setup){:target="_blank"} do Google Pay.<br>
...
Para integrar o Google Pay em seu aplicativo, siga todos os passos indicados no [Tutorial de Implementação](https://developers.google.com/pay/api/android/guides/tutorial){:target="_blank"} do Google Pay.
<br/><br/>
...
A partir do `PaymentData`, obtém-se o objeto `PaymentMethodToken`, através da chamada do método `getPaymentMethodToken()`. [Clique aqui](https://developers.google.com/android/reference/com/google/android/gms/wallet/PaymentData){:target="_blank"} para mais informações.

Na sequência, deve-se obter a _string_ que contém tokens de pagamento a partir do método `GetToken()` do objeto `PaymentMethodToken`. [Clique aqui](https://developers.google.com/android/reference/com/google/android/gms/wallet/PaymentMethodToken){:target="_blank"} para mais informações.

A *string* obtida no passo anterior possui uma estrutura como a mostrada a seguir. [Clique aqui](https://developers.google.com/pay/api/android/guides/resources/payment-data-cryptography){:target="_blank"} para mais informações.
...
Para mais informações, consulte o [Manual do Pagador](https://braspag.github.io/manual/braspag-pagador){:target="_blank"}.

### Resposta

A resposta de autorização da wallet terá os mesmos campos apresentados em nossa documentação do [Pagador](https://braspag.github.io/manual/braspag-pagador?json#resposta){:target="_blank"}, porém com a adição do nó `Payment.Wallet` repetindo os mesmos campos utilizados na autorização, como descrito acima.
...
> **Atenção**: É necessário que a loja já possua cadastro e integração Samsung Pay, caso contrário não será possível a integração com a API. Para mais informações, consulte a documentação da [Samsung Pay](https://pay.samsung.com/developers){:target="_blank"}.
...
| `WalletKey`         | Chave criptográfica que representa os dados do cartão. Consultar a tabela [WalletKey](https://braspag.github.io//manual/ewallets#walletkey) para mais informações.                    | Texto | 255     | Ver a tabela [WalletKey](https://braspag.github.io//manual/braspag-pagador#walletkey){:target="_blank"}              |
