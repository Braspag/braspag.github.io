...
1. Autenticação via Java Script: ideal para implementação em [websites](https://braspag.github.io/manual/autorizacao-com-autenticacao)
2. Autenticação via SDK Android **(em breve)**: ideal para implementação [in-app Android]
3. Autenticação via SDK IOS **(em breve)**: ideal para implementação em [in-app IOS]
