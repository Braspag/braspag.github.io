...
Obtenha o valor do *Authorization* concatenando o valor do "ClientID", sinal de dois-pontos (":") e "ClientSecret". Exemplo:
...
<aside class="notice">O caracter **#** indicado no campo deve ser substituído pelo número que representa o índice do item. Exemplo: `bpmpi_item_1_productName` representa o nome do item **1** do carrinho</aside>
...
Após a conclusão da autenticação do portador do cartão, a loja deve submeter a transação ao processo de autorização, enviando os dados de autenticação no modelo de "autenticação externa" (nó `ExternalAuthentication`), tanto para MPI Braspag quanto para MPI externo.
