...
| bpmpi_cardtype | Tipo do cartão a ser autenticado. No caso do cartão múltiplo, deverá especificar um dos tipos, Credit ou Debit | Credit – Cartão de Crédito<br>Debit – Cartão de Débito | Sim | Sim |
...
| bpmpi_merchantNewCustomer | Indica se o comprador é um cliente novo ou não | Booleano<br>true – comprador novo<br>false – comprador que já fez uma compra anteriormente | Não | Sim |
| bpmpi_paymentAccountDate | Indica a data de quando o cartão foi adicionado na conta do comprador | Numérico<br>-1 – não foi adicionado (guest account)<br>0 – adicionado durante esta transação<br>DDMMAAAA – nenhuma das alternativas acima | Não | Sim |
...
| bpmpi_billTo_customerAccountChangeDate | Indica a data de quando houve a última alteração na conta do comprador | Numérico<br>-1 – guest account<br>0 – adicionado durante esta transação<br>DDMMAAAA – data da última alteração | Não | Sim |
| bpmpi_billTo_customerAccountCreateDate | Indica a data de quando houve a criação da conta do comprador | Numérico<br>-1 – guest account<br>0 – adicionado durante esta transação<br>DDMMAAAA – data da criação  | Não | Sim |
| bpmpi_billTo_customerAccountPasswordChangeDate | Indica a data de quando houve a alteração de senha da conta do comprador | Numérico<br>-1 – guest account<br>0 – adicionado durante esta transação<br>DDMMAAAA – data da última alteração de senha  | Não | Sim |
...
| bpmpi_shipTo_destinationCode | Indica o tipo do destino do endereço de entrega | Numérico [2 posições]01- Mesmo endereço que o de cobrança<br>02- Endereço diferente do de endereço de cobrança, mas previamente verificado<br>03- Endereço diferente do endereço de cobrança<br>04- Retirar na loja<br>05- Digital goods<br>06- Viagem, ingressos que não há entrega shipped<br>07- Outros  | Não | Sim |
...
| bpmpi_deviceFingerprintData_#_provider | Nome do provedor do Device Finger Print | Alfanumérico [até 32 posições] cardinal<br>inauth<br>threatmetrix| Não | Sim |
| bpmpi_reorder | Indica se é a primeira compra ou não | Numérico [1 posição]0 – primeira compra<br>1 – compra recorrente | Não | Sim |
...
| bpmpi_productCode | Tipoda compra | ACC: Hotelaria<br>ACF: Financiamento de conta<br>CHA: Check acceptance<br>DIG: Digital Goods<br>DSP: Dispensação de dinheiro<br>GAS: Combustível<br>GEN: Varejo geral<br>LUX: Artigos de luxo<br>PAL: recarga<br>PHY: compra de mercadorias<br>QCT: Transação quase-dinheiro<br>REN: Alugue de Carros<br>RES: Restaurante<br>SVC: Serviços<br>TBD: Outros<br>TRA: Turismo | Não | Sim |
| bpmpi_shipAddressUsageDate | Indica a data de quando houve a primeira utilização do endereço de entrega | Numérico<br>-1 – guest account<br>0 – utilizado pela primeira vez nesta transação<br>DDMMAAAA – data da criação  | Não | Sim |
...
| bpmpi_alternateAuthenticationMethod| Método de autenticação do comprador na loja | 01- Não houve autenticação<br>02- Login da própria loja<br>03- Login com ID federado<br>04- Login com autenticador FIDO | Não | Sim |
...
| bpmpi_marketingOptIn | Indica se o comprador aceitou receber ofertas de marketing | Booleano<br>true – sim<br>false – não  | Não | Sim |
...
| bpmpi_defaultCard | Indica se o cartão/token utilizado é uma opção padrão | Booleano<br>true – sim<br>false – não/li></ul>  | Não | Sim |
