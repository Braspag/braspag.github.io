...
|`Tid`|Id da transação na adquirente <br/> Obs.: Caso você não possua integração com o Pagador Braspag, não terá como enviar o campo `BraspagTransactionId`. Nesse caso, tem a opção de enviar este campo, acompanhado dos campos `Nsu`, `AuthorizationCode` e `SaleDate`|string|não|20|
|`Nsu`|Número sequencial único da transação na adquirente <br/> Obs.: Caso você não possua integração com o Pagador Braspag, não terá como enviar o campo `BraspagTransactionId`. Nesse caso, tem a opção de enviar este campo, acompanhado dos campos `Tid`, `AuthorizationCode` e `SaleDate`|string|não|10|
|`AuthorizationCode`|Código de autorização da transação na adquirente <br/> Obs.: Caso você não possua integração com o Pagador Braspag, não terá como enviar o campo `BraspagTransactionId`. Nesse caso, tem a opção de enviar este campo, acompanhado dos campos `Tid`, `Nsu` e `SaleDate`|string|não|10|
|`SaleDate`|Data da autorização da transação da transação na adquirente <br/> Obs.: Caso você não possua integração com o Pagador Braspag, não terá como enviar o campo `BraspagTransactionId`.Nesse caso, tem a opção de enviar este campo, acompanhado dos campos `Tid`, `Nsu` e `AuthorizationCode`|datetime|não|-|
