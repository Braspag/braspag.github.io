...
The authorization response returned by the wallet will have the same fields shown in our [Pagador] documentation (https://braspag.github.io//en/manual/braspag-pagador#response), but with the addition of the `Payment.Wallet` node repeating the same fields used in the authorization, as described above.
