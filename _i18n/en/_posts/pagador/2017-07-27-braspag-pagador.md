---
layout: redirect
redirect: https://docs.cielo.com.br/gateway-en/docs/
...
You are able to export card tokens (`CardToken`) stored in Braspag vault to use in other payment providers and also to import card tokens created in any other tokenization service to Braspag.

* **Exporting a card token**: when the merchant exports their card tokens token to be used in other payment providers. The exporting process may be charged by Braspag, and there are other requirements such as signing a responsability agreement, inform the PCI DSS certificate for the token destination entity, prepare the SFTP environment, provide a public PGP key and inform the credentials. For further information about this process, plase refer to [Exportação de tokens do Cartão Protegido](https://suporte.braspag.com.br/hc/pt-br/articles/19914071862811-Exporta%C3%A7%C3%A3o-de-tokens-do-Cart%C3%A3o-Protegido){:target="_blank"} or contact Support;
...
> For security reasons, Braspag uses PGP encryption. [Download the public key](https://braspag.github.io/attachment/chave-publica.txt){:target="_blank"} Braspag/Cielo for importing card tokens.
...
You can query a transaction for up to 18 months after it was created. Here are different ways to perform a sale or transaction query, depending on its lifetime:

|LIFETIME|FORM OF QUERY|
|---|---|
|Up to 5 months|Via API or Admin Braspag panel.|
|From 5 to 18 months|Via Admin Braspag panel with the “History” option selected.|
